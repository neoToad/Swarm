...
players_dict = {
    "Player01":
        {'color': RED, 'base': Base(screen, (SCREEN_WIDTH / 2, 100), RED, p1_units, P1_ADDUNIT), 'group': p1_units, 'respawn event': P1_ADDUNIT},
    "Player02":
        {'base': Base(screen, (150, SCREEN_HEIGHT / 2), BLUE, p2_units, P2_ADDUNIT), 'group': p2_units, 'respawn event': P2_ADDUNIT},
    "Player03":
        {'base': Base(screen, (SCREEN_WIDTH / 2, SCREEN_HEIGHT - 100), GREEN, p3_units, P3_ADDUNIT), 'group': p3_units, 'respawn event': P3_ADDUNIT},
    "Player04":
        {'base': Base(screen, (SCREEN_WIDTH - 150, SCREEN_HEIGHT / 2), WHITE, p4_units, P4_ADDUNIT), 'group': p4_units, 'respawn event': P4_ADDUNIT}
}
...
def spawn_units():
    for player, add_command in add_player_units:
        ...
        if event.type == add_command:
            new_enemy = Unit(screen, players_dict[player]['base'], players_dict[player]['base'].color, p1_speed,
